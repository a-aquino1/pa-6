...
```{r}
#| label: commenting-proportion-votes
unvotes |> 
  count(country, vote) |> #counting by country and votes
  group_by(country) |> #grouping by countries
  mutate(total = sum(n)) |> #creating a new column called "total" which is the sum of n 
  mutate(prop_vote = n/total) |> #creating a new column called "prop_vote" which n divided by the total
  filter(country %in% c("United States", "Canada", 
                        "Germany", "France",
                        "Italy", "Japan",
                        "United Kingdom")) |> #filtering to keep in the provided countries
  ggplot(aes(x = country, y = prop_vote, 
             fill = vote)) + #creating plot proportion of votes per country
  geom_col(position = position_stack()) + #positioning columns
  labs(x = "Group of Seven Countries",
       y = "Proportion of Votes",
       title = "Voting Record of the G7",
       fill = "Vote") + #assigning titles and names to axis on graph
  theme_minimal() +  #background of graph
  scale_fill_viridis_d(end = 0.8) + #color on the actual graph
  coord_flip() #flips the coordinate system
```

> Describe what the graph above demonstrates above UN voting records for the G7 
  
  The graph above demonstrates the proportion per country of how many people voted "yes", "no", and refused to vote.
...
```{r}
#| label: distinct-function
roll_calls |> 
  distinct(short)
#ran the given code
```

> Prints out the row named short from the roll_call column.
...
> WHat countries are voting? What are they voting on? 
...
> The information above gives a general idea of what the countries are voting on but there are some descriptions that are not detailed enough.
...
```{r}
#| label: dim-issues-and-roll_calls
dim(issues)
dim(roll_calls)
```


> They do not match as it indicates they have different voting trends
...
#| label: left-join
roll_calls |> 
  left_join(issues, by = "rcid") #joining to the left table
```

```{r}
#| label: right-join
roll_calls |> 
  right_join(issues, by = "rcid") #joining to the right table
```

```{r}
#| label: full-join
roll_calls |> 
  full_join(issues, by = "rcid") #joining both tables completely
```

```{r}
#| label: inner-join
roll_calls |> 
  inner_join(issues, by = "rcid") #joining tables based on related columns
```
...
> We would use the inner join to join the related columns for both tables and/or the left join as the 'issues' table is what the records are associated with.
...
roll_calls |> 
  ...
  mutate(issue = case_when(
    ...
    issue == "Palestinian conflict" ~ "Palestinian Conflict")) |>   #the issues are being assigned new names
  ...
  count(year, issue) |> #it uses the issues by the year they occur
  ggplot(aes(x = year, y = n, group = issue)) + #creating the plot based off the year by the number of votes
  geom_line(aes(color = issue)) + #creating a different color line per issue
  labs(x = "Year", y = "Number of Votes",
...
> A lot of the issues have similar trends as time goes on. Who are the political leaders at the time?
